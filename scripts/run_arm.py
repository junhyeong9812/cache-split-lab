...
# 기본 4.0 은 히트율 페이즈용. 포화 페이즈(Phase 0/4/D)는 부하 생성기 천장 아래로
# 조인 값을 쓴다 — env 로 오버라이드해야 문서의 배분(0.5 → 앱 0.2×2 + redis 0.1)이
# 이 파일만으로 재현된다: CPU_TOTAL=0.5 python3 scripts/run_arm.py d-redis …
CPU_TOTAL     = float(os.environ.get("CPU_TOTAL", "4.0"))   # BPC 16스레드 중 앱 몫
...
ARMS = {
    ...
    # arm D — 캐시가 공유 redis 하나(shared): 용량은 나누지 않고 총량 그대로,
    # CPU 는 redis 가 총량의 REDIS_FRAC 을 가져가고 앱 둘이 나머지를 나눈다.
    "d-redis":      {"nodes": 2, "profile": "two-node,redis", "shared": True},
}

# arm D 에서 redis 의 CPU 몫 — **총 자원 고정 안에서** (2026-07-19 사용자 결정).
# 0.2 인 이유: redis 의 일은 요청당 Lua 1회(캐시 연산만)고 HTTP·JSON 은 앱 몫이라
# 앱보다 가볍다. 단일 스레드라 CPU 를 더 줘도 코어 1개 이상은 못 쓴다.
REDIS_FRAC = 0.2
...
def per_node(arm):
    ...
    n = ARMS[arm]["nodes"]
    shared = ARMS[arm].get("shared", False)
    if not shared and CACHE_TOTAL % n != 0:
    ...
    env = {
        # shared(arm D): 캐시가 하나라 나누지 않는다 — 총량이 곧 그 캐시의 용량.
...
def main():
    ...
    if ARMS[a.arm].get("shared"):
        print(f"   공유 캐시 {pn['APP_CACHE_CAPACITY']} (redis, CPU {pn['REDIS_CPUS']}) "
              f"| 앱 CPU {pn['APP_CPUS']} × {ARMS[a.arm]['nodes']} | 스레드 {pn['THREADS_MAX']}", flush=True)
    else:
